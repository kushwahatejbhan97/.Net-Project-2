...
namespace LocalPandit.Admin
{
    public partial class Add_tag : System.Web.UI.Page
    {
        ...
        protected void DetailsView1_ItemInserting(object sender, DetailsViewInsertEventArgs e)
        {
            try
            {
                TextBox Tagname = (TextBox)DetailsView1.FindControl("TxtTagName");
                string urlt = Tagname.Text.Replace("&", "-").Replace(" ", "-").Replace("  ", "-").Replace("/", "-").Replace("+", "-").Replace("%", "-").Replace("$", "-").Replace("(", "-").Replace(")", "-").Replace("'", "-").Replace("--", "-"); 
                SqlDataSource1.InsertParameters.Add("urlt", urlt);
        ...
        private void uploadimage(int id)
        {
            try
            {
                ...
                if (id > 0)
                {
                    string fn1 = dalclass.InsertImage("\\upload_tag\\icon\\", 2);
                    string SaveLocation1 = Server.MapPath("~/upload_tag") + "\\icon\\" + fn1;
                    ...
                    if ((FUpBanner.PostedFile != null) && (FUpBanner.PostedFile.ContentLength > 0))
                    {
                        ...
                        dalclass.UpdateTagIConImageID(fn1, id);
