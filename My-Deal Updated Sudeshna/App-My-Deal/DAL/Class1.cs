...
namespace DAL
{
    public class Class1
    {
        ...
        public string Webotp()
        {
            string al = "1723";
            con = new SqlConnection(commm.GetConnectionString());
            cmd = new SqlCommand("SELECT  WebOTP FROM NewListing_Website_User", con);
        ...
        public ArrayList Existing_phone(string mobile)
        {
            ...
            con = new SqlConnection(commm.GetConnectionString());
            cmd = new SqlCommand("SELECT UserId , Name, Email  FROM  NewListing_Website_User WHERE  (Mobile = '" + mobile + "')", con);
            try
            {
                con.Open();
                SqlDataReader dr = cmd.ExecuteReader();
        ...
        public void AddPersonal(int clientId, string text1, string text2)
        {
            throw new NotImplementedException();
        }

        public int AddClient(string text)
        {
            throw new NotImplementedException();
        }
        ...
        public DataTable SelectCompnyBykeysearch(string keysearch)
        {
            DataTable OrderTable = new DataTable();
            con = new SqlConnection(commm.GetConnectionString());
            try
            {
                con.Open();
                string query = "SELECT [CompanyId], [CompanyName], [Mobile], [Password], [VerMobile], [CatId], [OwnerName], [YearEstablish], [LandLine], [Email], [Website], [Location], [Address], [City], [Map], [RegsitrationDate], [KeyWord], [Enable], [TotalReview], [Latitude], [Longitude], [url], [ListingType], [Priority], [Tags], [ListingImageID] FROM [NewListing_Website_listing_tbl] WHERE ([KeyWord] like '%" + keysearch + "%' )";
                SqlDataAdapter da = new SqlDataAdapter(query, con);
                da.Fill(OrderTable);
