...
namespace AppMyDeal
{
    public partial class Category : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                string caturl = Page.RouteData.Values["CatUrl"].ToString();
                int catId = dalclass.getCatidbyUrl(caturl);
                SqlDbPopulartag.SelectCommand = "SELECT [TagId], [Catid], [Tags],[urlt],[CatIConImageID] FROM [NewListing_Website_tags_tbl]  WHERE ([Catid] = '" + catId + "') ORDER BY TagId DESC";
                SqlDbPopulartag.DataBind();
                DataTable ListingTable = dalclass.SelectCompnyByCatName(catId);
                RepeaterPopularCat.DataSource = ListingTable;
                RepeaterPopularCat.DataBind();


                //string catName = Page.RouteData.Values["CatUrl"].ToString();

                //DataTable ListingTable = dalclass.SelectCompnyByCatName(catName);
                //RepeaterPopularCat.DataSource = ListingTable;
                //RepeaterPopularCat.DataBind();
            }
