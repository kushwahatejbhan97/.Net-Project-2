...
{
    public partial class list_CatId_wise : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    int catId = Int32.Parse(Request.QueryString["catId"].ToString());
                    if (location != null && location != "" && keyword != null && keyword != "")
                    {
                        SqlDataSourceListProperty.SelectCommand = "SELECT [CompanyId], [CatId], [CompanyName], [OwnerName], [YearEstablish], [Mobile], [LandLine], [Email], [Website], [Location], [Address], [City], [Map], [RegsitrationDate], [KeyWord], [Enable], [TotalReview], [url], [Password], [CompImgID] FROM [NewListing_Website_listing_tbl] WHERE (([City] = '" + location + "') AND ([CatId] = '" + catId + "' ) AND [Enable] = 'true') ORDER BY Priority ASC";
                        lblLocation.Text = location;
                        lblkey.Text = keyword;
                        int count = dalclass.Select_Company_List_count(location, catId);
